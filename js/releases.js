            ...
            const pinnedSection = pinnedHtml ? `
                <div class="list-section-wrap">
                    <div class="list-section-header" onclick="toggleListSection('pinned')">
                        <span><i class="fas fa-layer-group" style="color:var(--accent);margin-right:6px;"></i> <b>Плейлисты</b>&nbsp;<span style="color:var(--text-dim);font-size:12px;">(закреплённые)</span></span>
                        <i class="fas fa-chevron-down list-section-chevron" id="chev-pinned"></i>
                    </div>
                    <div class="list-section-body list-section-open" id="body-pinned">${pinnedHtml.replace(/^.*?<div class="pl-profile-grid">|<\/div>\s*<\/div>\s*$/gs,'')}</div>
                </div>` : '';
