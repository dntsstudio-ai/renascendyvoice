    ...
    window.sendSuggestion = async () => {
        ...
        try {
            if (window.emailjs) await emailjs.send(EMAILJS_CONFIG.serviceId, EMAILJS_CONFIG.templateSuggest,
                { to_email:'renascendivoice@gmail.com', from_name:senderName, from_email:senderEmail||'нет', title, media_type:type, link:link||'не указана', reason, date:new Date().toLocaleString('ru') }, EMAILJS_CONFIG.publicKey);
